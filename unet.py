...
# Liver Data set class to preprocess data 
...
# model.load_state_dict(torch.load(r'.\unet_liver_segmentation.pth')) # continue training the used model

# Define the loss function and optimizer
criterion = nn.BCEWithLogitsLoss()  # Use for binary segmentation
...
for epoch in range(num_epochs):
    ...
    for images, masks in train_loader:
        ...
        # Forward pass the forward method is being called implicitly whenever you pass an input tensor to the model, which is the default behavior in PyTorch.
        ...
        # Backpropagation
        loss.backward() #caculate gradients for optimizer
        optimizer.step()
...
# Save the model
torch.save(model.state_dict(), "unet_liver_segmentation.pth")
...
image_path2 = r".\Liver_Medical_Image _Datasets\Images\113.png"
image_path3 = r".\Liver_Medical_Image _Datasets\Images\389.png"
output_mask = segment_single_image(model, image_path, transform, device)
output_mask2 = segment_single_image(model, image_path2, transform, device)
output_mask3 = segment_single_image(model, image_path3, transform, device)
...
Image.fromarray((output_mask2 * 255).astype(np.uint8)).save("segmented_liver_image2.jpg")
Image.fromarray((output_mask3 * 255).astype(np.uint8)).save("segmented_liver_image3.jpg")
